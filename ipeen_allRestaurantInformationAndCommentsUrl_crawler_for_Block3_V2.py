...
def convertGrading(string):
    if (string in ('非常好','非常滿意')):
        return 5
    elif (string in ('很好','很滿意')):
        return 4
    elif (string in ('好','滿意')):
        return 3
    elif (string in ('普通','一般')):
        return 2
    elif (string in ('差','不滿意')):
        return 1
    else:
        return None 
...
#爬取食記裡所需要的資料
def comment_crawler(comment_url):

    try: 
        res_comment_page = r.get(comment_url)
    except:
        print('can not get comment_page')
        return None
    try:
        print(res_comment_page.status_code)
        res_comment_page.encoding="utf-8"
        soup = bs(res_comment_page.text, 'lxml')
        
        #發表日期
        created_date = soup.select('.date > span')[0].text
            
        #美味度
        delicious_str = soup.select('.rating > dd')[0].text
        delicious = convertGrading(delicious_str)
        
        #服務品質
        service_quality_str = soup.select('.rating > dd')[1].text
        service_quality = convertGrading(service_quality_str)
        
        #環境氣氛
        dining_environment_str = soup.select('.rating > dd')[2].text
        dining_environment = convertGrading(dining_environment_str)
        ...
        return author_id,created_date,delicious,service_quality,dining_environment,read_int,share_int
...
def reduce_allRestaurantInformationAndCommentsUrl_SplitBlock_function(q,startTime):
    try:
        rf = open('all_restaurant_list_block3.txt', 'r',encoding='utf8')
        restaurant_str = rf.read()
        #print(restaurant_str)
        
    except FileNotFoundError:
        print("'[ERROR]No such file or directory: 'all_restaurant_list_block3.txt'")
        raise
    ...
    if (restaurant_str==''):
        ...
        with open('success_all_InformationAndCommentsUrl.txt', 'w', encoding='utf8') as wsf:
            wsf.write('success')
        os.remove('all_restaurant_list_block3.txt')
...
if __name__ == '__main__':   
    ...
    try:
        ...
        with open('success_all_restaurant_list_block3.txt', 'w', encoding='utf8') as wsf:
            wsf.write('success')
        os.remove('all_restaurant_list_block3.txt')
        
    except AWSTimeLimitError:
        with open('all_restaurant_list_block3.txt', 'w', encoding='utf8') as wf:
            while not q.empty():                    
                page = q.get()
                wf.write(page + '\n')  
    except HTTPError:
