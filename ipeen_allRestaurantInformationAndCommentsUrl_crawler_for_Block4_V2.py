...
def reduce_allRestaurantInformationAndCommentsUrl_SplitBlock_function(q,startTime):
    try:
        rf = open('all_restaurant_list_block4.txt', 'r',encoding='utf8')
        restaurant_str = rf.read()
        #print(restaurant_str)
        
    except FileNotFoundError:
        print("'[ERROR]No such file or directory: 'all_restaurant_list_block4.txt'")
        raise
    ...
    if (restaurant_str==''):
        ...
        with open('success_all_InformationAndCommentsUrl.txt', 'w', encoding='utf8') as wsf:
            wsf.write('success')
        os.remove('all_restaurant_list_block4.txt')
...
if __name__ == '__main__':   
    ...
    try:
        ...
        with open('success_all_restaurant_list_block4.txt', 'w', encoding='utf8') as wsf:
            wsf.write('success')
        os.remove('all_restaurant_list_block4.txt')
        
    except AWSTimeLimitError:
        with open('all_restaurant_list_block4.txt', 'w', encoding='utf8') as wf:
            while not q.empty():                    
                page = q.get()
                wf.write(page + '\n')  
    except HTTPError:
