...
def reduce_AllPagesListSplitBlock_function(q,startTime):
    try:
        rf = open('all_pages_list_block3.txt', 'r',encoding='utf8')
        pages_str = rf.read()
        
    except FileNotFoundError:
        print("'[ERROR]No such file or directory: 'all_pages_list_block3.txt'")
        raise
        
    if (pages_str==''):
        print('no url!')
        rf.close()
        with open('success_all_pages_list_block3.txt', 'w', encoding='utf8') as wsf:
            wsf.write('success')
        os.remove('all_pages_list_block3.txt')
...
if __name__ == '__main__':   
    ...
    while not q.empty():
        try:
            ...
            with open('all_pages_list_block3.txt', 'w', encoding='utf8') as wf:
                page = q.get()
                wf.write(page + '\n')
    if q.empty():
        with open('success_all_pages_list_block3.txt', 'w', encoding='utf8') as wsf:
